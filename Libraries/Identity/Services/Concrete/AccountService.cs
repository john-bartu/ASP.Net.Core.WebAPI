...
namespace Identity.Services.Concrete
{
    public class AccountService : IAccountService
    {
        ...
        public async Task ForgotPasswordAsync(ForgotPasswordRequest request, string uri)
        {
            ...
            {
                Body = $"You have to send a request to the '{endpointUri}' service with reset token - {code}",
                To = request.Email,
        ...
        public async Task<BaseResponse<string>> ResetPasswordAsync(ResetPasswordRequest request)
        {
            ...
            if (result.Succeeded)
            {
                return new BaseResponse<string>(request.Email, message: $"Password reset.");
            }
            else
            {
                throw new ApiException($"Error occured while password reset. Please try again.");
            }
        }
